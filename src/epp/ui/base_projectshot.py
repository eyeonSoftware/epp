...
from epp.__INFO__ import DEBUG
...
class SaveAsDialog(Ui_dlgSaveAs, QDialog):
    ...
    def update_project(self):
        ...
        for item in sorted(glob.glob(os.path.join(self.PROJECTROOT, "*")), cmp=lambda x,y: cmp(os.path.basename(x).lower(), os.path.basename(y).lower())):
            if os.path.isdir(item):

                # TODO: Optional - only folders with config xml file
                self.cmbProject.addItem(os.path.basename(item), item)
